...
function gameStatusReceived(err, data){
	if(err){
		...
		var haveIJoined = _.find(data.participants, function(o) { return o.teamId.toUpperCase() === config.team.toUpperCase(); }) != null;
        switch(data.status){
			case 'Joining':
				//joining phase
				myGuessTracker = {};
			...
			case 'Running':
				if(!haveIJoined){
					...
					var amIAlive = _.find(data.participants, function(o) { return o.teamId.toUpperCase() === config.team.toUpperCase() && o.isAlive === true;}) != null;
					if(!amIAlive){
